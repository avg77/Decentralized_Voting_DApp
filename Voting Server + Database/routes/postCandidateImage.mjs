...
router.post('/postCandidateImage', authentication, multerConfig.uploadCandidate, async (req, res) => {
  try {
    ...
    const pinataResult = await pinata.pinFileToIPFS(pinataStream, {
      pinataMetadata: { name: `candidate-${accountAddress}` }
    });

    const filecoinCid = await uploadToLighthouse(buffer, `candidate-${accountAddress}`);
    ...
    console.error("Candidate image upload failed:", error);
